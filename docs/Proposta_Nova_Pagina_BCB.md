...
- **Chave de junção:** o CNPJ (raiz, 8 dígitos) do arquivo BCB é idêntico em formato ao `CNPJ` já usado em `dAdministradora`/`ft_Consorcios`. Confirmado por amostragem (ex.: Honda 45441789, Bradesco 52568821). O join é direto, sem tratamento adicional.
...
- As **5 maiores administradoras por Patrimônio Líquido concentram 58,5%** de todo o PL do sistema; as 10 maiores, 75,3%. Índice HHI (PL) = **1.469**, o que indica um mercado moderadamente concentrado.
...
- Ranking por receita de taxa de administração: BB (R$ 932 mi), Bradesco (R$ 845 mi), Honda (R$ 644 mi), Itaú (R$ 548 mi), Ademicon (R$ 524 mi). A ordem é diferente da do PL, o que mostra que porte patrimonial e faturamento corrente não seguem o mesmo padrão.
...
- Duas administradoras de pequeno porte (M.I6, Brquality) apresentam PL superior ao Ativo Total apurado. É inconsistência pontual de baixa materialidade, provavelmente por composição de contas não totalmente comparável entre instituições; não compromete os agregados do sistema, mas deve constar como nota técnica caso a tela seja publicada.
...
- Esta é uma leitura de **uma única competência (03/2026)**; para conclusões de tendência (não só de fotografia) seria necessário acumular 2 a 3 competências, o que o portal do BCB permite.
...
A diferença desta tela para o restante do material é qualitativa, não só de conteúdo: as páginas atuais mostram desempenho **operacional** (cotas, contemplação, inadimplência), que é informação já pública e de leitura neutra. Esta nova tela exporia, por administradora nomeada, **resultado (lucro/prejuízo) e patrimônio líquido**, dado também público (BCB), mas de leitura mais sensível institucionalmente, especialmente a lista de administradoras com resultado negativo no período. Recomendo que a presidência avalie especificamente esse ponto antes da publicação, independentemente da qualidade técnica da análise.

---

## 5. Próximos passos sugeridos

Caso a presidência aprove o conceito, o próximo passo é a construção efetiva da página no arquivo Power BI (medidas DAX, nova tabela de fatos financeiros, visuais). Ela ainda não foi iniciada, aguardando sinalização para prosseguir.
